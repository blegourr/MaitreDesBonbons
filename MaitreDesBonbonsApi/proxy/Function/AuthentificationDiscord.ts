...
import { getUserByUserID, createUser, updateUserByUserID } from '../../db/Fucntion/User';
/**----------------------------------------------------
 *                     CONFIG
 *-----------------------------------------------------
 */
 ...
 *-----------------------------------------------------
*/
export async function verifyTokenMiddleware(req: Request, res: Response): Promise<Boolean> {
 ...
 */
 export async function authDiscordAcount(req: Request, res: Response): Promise<void> {
  ...
  // Mise en cookie de l'id de l'utilisateur
  res.cookie('my_user_id', user.id, {
    httpOnly: false,
    maxAge: 7 * 24 * 60 * 60 * 1000, // 7 jours
  });
